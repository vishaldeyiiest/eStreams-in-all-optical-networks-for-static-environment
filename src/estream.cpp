#include <bits/stdc++.h>
using namespace std;
//char fbpe[100] = "../paths2/nsfnet5/igstream/bpe1.txt";
char fpath[100] = "../paths/nsfnet/gstream/4.txt";
char test[100] = "../testing/nsfnet5/1.txt";
void display(vector<int> a)									//display the vector
{
	//ofstream fo(fbpe,ios_base::app);
...
vector<int> get_shortestpath(int src, int dest)			//choosing the longest path as backup path
{
	ifstream fp(fpath);
	string line;
	vector<vector<int> > a;
...
vector<int> check_longest(vector<pair<int,int> > r)			//finding the request giving the longest backup path
{
	...
	{
		p = get_shortestpath(r[i].first, r[i].second);
		if (p.size() > max)
		{
			max = p.size();
			pm = p;
	...
	cout<<"Request : "<<longest.first<<" "<<longest.second<<endl;
...
pair<pair<int,int>,pair<int,int> > first_tworequests(vector<pair<int,int> >  set_pairs)
{
	...
	for(unsigned int i = 0; i < a.size()-1; i++)
	{
		...
		for(unsigned int j = 0; j < a.size(); j++)
		{
			...
			if(t > max && a[i].front() != a[j].front() && a[i].back() != a[j].back())
			{
				max = t;
				p = i, q =j;
			}
...
int main()
{
	...
	{
		cout<<"Backup path: ";
		vector<int> xx = get_shortestpath(set_pairs[i].first, set_pairs[i].second);
		display(xx);
		count += xx.size()-1;
	}	
	# if 1
	cout<<"Backup paths chosed for stream: "<<endl;
	for(unsigned int i = 0; i < bpath.size(); i++)
	{
		for(unsigned int j = 0; j < bpath[i].size(); j++)
			cout<<bpath[i][j]<<" ";
		cout<<endl;
		count += bpath[i].size()-1;
	}
	#endif
	t = clock() - t ;
	count -= gain;
